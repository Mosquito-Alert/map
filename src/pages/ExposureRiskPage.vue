<template>
  <inner-drawer :title="$t('exposure_risk')" :init-collapsed="$q.platform.is.mobile && formIsValid">
    <q-select
      ...
      :label-color="selectedSpeciesCode ? 'primary' : 'rgba(0, 0, 0, 0.6)'"
    ...
    <q-input
      ...
      :label-color="selectedDate ? 'primary' : 'rgba(0, 0, 0, 0.6)'"
      :disable="!selectedSpeciesCode"
    ...
    <div class="q-mt-lg" v-if="formIsValid">
      ...
      <div v-show="selectedSpeciesCode" class="q-mt-xl">
        <div class="row">
          <span class="text-weight-light text-uppercase text-grey-7">{{
            $t('layer_controls')
          }}</span>
          <q-space />
  ...
  <ExposureRiskMapLayer
    v-if="formIsValid && selectedSpeciesCode && selectedDate"
    :level="1"
    :visible="visibleLayer"
    :opacity="opacityLayer"
    :species-code="selectedSpeciesCode"
    :date="selectedDate"
    :palette="palette"
    :filters="localFilter"
  />
...
const selectedDateString = computed(() => {
  return selectedDate.value?.toLocaleDateString($q.lang.isoName || 'en', {
    year: 'numeric',
    month: 'long',
  })
})
...
const minYearMonthPickerValue = computed(() =>
  speciesModelManifest.value
    ? `${speciesModelManifest.value.fromYear}/${speciesModelManifest.value.fromMonth}`
    : undefined,
)
const maxYearMonthPickerValue = computed(() =>
  speciesModelManifest.value
    ? `${speciesModelManifest.value.toYear}/${speciesModelManifest.value.toMonth}`
    : undefined,
)
...
onMounted(() => {
  ...
  cdn
    .get('static/models/global_minimal_model_estimates/model_manifest.csv')
    .then((resp) => {
      // Read csv manifest
