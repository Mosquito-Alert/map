<template>
  <inner-drawer
    :title="$t('discoveries')"
    :init-collapsed="$q.platform.is.mobile && !!selectedSpeciesCode"
  >
    <q-select
    ...
    <div v-show="selectedSpeciesCode" class="q-mt-xl">
      <div class="row">
        ...
        <q-btn-group flat stretch class="bg-grey-1">
          ...
          <q-btn
            :loading="downloadProgress.loading"
...
async function downloadFile() {
  ...
  try {
    ...
    exportFile(`discoveries_${selectedSpeciesCode.value}.zip`, response.data, {
