<template>
      ...
      <ol-feature>
        ...
        <ol-style>
          <ol-style-text
            ...
            textBaseline="bottom"
            :text="String.fromCodePoint(0xf3c5)"
  ...
  <ObservationDetailDrawer
    v-if="report && report.type === ReportType.Observation"
    :observation="report"
    @close="emit('close')"
  />
  <BreedingSiteDetailDrawer
    v-else-if="report && report.type === ReportType.BreedingSite"
    :breedingSite="report"
    @close="emit('close')"
  />
  <BiteDetailDrawer
    v-else-if="report && report.type === ReportType.Bite"
    :bite="report"
    @close="emit('close')"
  />
</template>
