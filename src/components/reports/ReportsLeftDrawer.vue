<template>
  ...
  >
    <q-list separator>
      ...
      <!-- MOSQUITO SELECTION -->
      <q-expansion-item
        :model-value="mosquitoSectionIsExpanded"
        dense
        expand-separator
      ...
      >
        <template v-slot:header>
          ...
          <q-item-section side v-if="mosquitoSelected.length">
            <q-btn
              flat
              round
              icon="fa fat fa-download"
              size="sm"
              :loading="downloadProgress.observations.loading"
        ...
        <q-card>
          <q-card-section class="column">
            <div class="row">
              <q-chip
                v-for="(item, index) in mosquitoLayers"
                :key="index"
                v-model:selected="item.enabled"
                class="no-shadow"
                :label="$t(item.name_key)"
                :color="item.enabled ? item.color : 'grey-3'"
                :text-color="item.enabled ? 'white' : 'grey-14'"
            ...
            <div class="row">
              <q-chip
                v-for="(item, index) in otherSpeciesLayers"
                :key="index"
                v-model:selected="item.enabled"
                class="no-shadow"
                :label="$t(item.name_key)"
      ...
      <!-- BREEDING SITES -->
      <q-expansion-item
        :model-value="breedingSitesSectionIsExpanded"
        dense
        expand-separator
        header-class="q-px-none"
      >
        <template v-slot:header>
          ...
          <q-item-section class="text-capitalize">
            <div class="row items-center">
              {{ $t('breeding_sites') }}
              <q-badge
                v-if="breedingSitesSelected.length"
                rounded
                class="q-ml-sm"
                color="grey-3"
                text-color="grey-14"
          ...
          <q-item-section side v-if="breedingSitesSelected.length">
            <q-btn
              ...
              icon="fa fat fa-download"
              size="sm"
              :loading="downloadProgress.breedingSites.loading"
        ...
        <q-card>
          <q-card-section>
            <q-chip
              v-for="(item, index) in breedingSiteLayers"
              :key="index"
              v-model:selected="item.enabled"
              class="no-shadow"
              :label="$t(item.name_key)"
        ...
        <q-item-section side>
          <div class="row items-center">
            <q-btn
              ...
              icon="fa fat fa-download"
              size="sm"
              :loading="downloadProgress.bites.loading"
              :percentage="downloadProgress.bites.percentage"
      ...
      <q-item
        dense
        class="q-px-none"
        :active="samplingEffortEnabled"
        active-class="text-sampling-effort"
      >
        <q-item-section avatar>
        ...
        <q-card class="bg-grey-1">
          <q-card-section>
            <q-chip
              ...
              text-color="white"
              icon="fa fat fa-tag"
              v-for="(tag, index) in tagsSelected"
              :key="index"
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const mosquitoSelected = computed(() => {
      ...
      return (Object.keys(layers) as Array<keyof typeof layers>).filter(
        (key) => layers[key].enabled,
      )
    })
    ...
    const breedingSitesSelected = computed(() => {
      return (
        Object.keys(breedingSiteLayers.value) as Array<keyof typeof breedingSiteLayers.value>
      ).filter((key) => breedingSiteLayers.value[key].enabled)
    })
        ...
        .list(
          ...
          },
          {
            onDownloadProgress: (event: AxiosProgressEvent) => {
              if (event.total) {
                downloadProgress.value.bites.percentage = Math.round(
    ...
    async function downloadObservations() {
      ...
      const taxonIds =
        mosquitoSelected.value == Object.keys(mosquitoLayers.value)
          ? undefined
      ...
      await observationsApi
        .list(
          ...
          },
          {
            onDownloadProgress: (event: AxiosProgressEvent) => {
              if (event.total) {
                downloadProgress.value.observations.percentage = Math.round(
                  (event.loaded * 100) / event.total,
                )
              }
            },
    ...
    async function downloadBreedingSites() {
      ...
      const breedingSiteTypesSelected = breedingSitesSelected.value
        .map((key) => {
      ...
      await breedingSitesApi
        .list(
          ...
          },
          {
            onDownloadProgress: (event: AxiosProgressEvent) => {
              if (event.total) {
                downloadProgress.value.breedingSites.percentage = Math.round(
                  (event.loaded * 100) / event.total,
                )
              }
            },
