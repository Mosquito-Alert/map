<template>
  ...
  >
    ...
    <q-item-section>
      ...
      <q-item-label v-if="report?.location?.display_name !== null" caption lines="2">
        <div class="row items-center no-wrap">
          ...
          <span class="col-auto" v-if="report">{{
            report?.location.display_name?.split(',')[0]
          }}</span>
    ...
    <q-item-section side top>
      <q-item-label v-if="report" caption
        >{{
          toRelativeTime(new Date(report.received_at), {
            style: 'narrow',
          })
        }}
      </q-item-label>
...
const photoUrl = computed(() => {
  if (props.feature.get('type') === ReportType.Observation) {
    ...
    return observation.photos.length > 0 ? observation.photos[0]!.url : ''
  } else if (props.feature.get('type') === ReportType.BreedingSite) {
    const breedingSite = report.value as BreedingSite
    return breedingSite.photos.length > 0 ? breedingSite.photos[0]!.url : ''
  }
  return undefined
})
