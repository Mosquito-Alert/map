<template>
  <BaseReportDetailDrawer
    ...
    :subtitle="subtitleWithStyle"
    :extra-items="extraItems"
    @close="$emit('close')"
  >
    <template #header>
      <PhotoSliderHeader
        v-if="observation.photos.length > 0"
        :photo-urls="observation.photos.map((photo) => photo.url)"
      />
    </template>
    <template #header-icons>
      <q-icon
        v-if="observation.identification?.result?.characteristics?.sex !== undefined"
        :name="
          observation.identification.result.characteristics.sex === SpeciesCharacteristicsSex.Male
            ? 'fa fa-mars'
            : 'fa fa-venus'
        "
        color="white"
      />
      <q-icon
        v-if="observation.identification?.result?.source === IdentificationTaskResultSource.Expert"
        name="fa fa-users"
        color="white"
      >
      ...
      <q-icon
        v-else-if="observation.identification?.result?.source === IdentificationTaskResultSource.Ai"
        name="fa fa-robot"
        color="white"
      >
        <q-tooltip anchor="top middle" self="center middle">
        ...
        <q-card flat>
          ...
          <q-card-section>
            <div class="row q-mb-sm">
              <span class="text-body2 text-uppercase text-grey-7"
                >{{ $t('confidence_level') }}:</span
              >
              <q-rating
                :model-value="observation.identification?.result?.is_high_confidence ? 3 : 2"
                :max="3"
                readonly
                color="primary"
                icon="fa fa-light fa-circle-check"
                size="xs"
                class="q-ml-xs"
              />
            </div>
