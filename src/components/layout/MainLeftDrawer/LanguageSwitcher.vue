<template>
  <q-btn-dropdown
    icon="fa fat fa-language"
    dropdown-icon="fa fat fa-angle-right"
...
export default {
  setup() {
    ...
    watch(locale, (newValue) => {
      ...
      modules[`../../node_modules/quasar/lang/${newValue}.js`]?.()
        .then((langModule) => {
    ...
    return {
      ...
      localeOptions: Object.keys(enabledLanguages).map((isoName) => ({
        value: isoName,
        label: languages.filter((lang) => lang.isoName === isoName)[0]?.nativeName,
      })),
    }
  },
}
